...
for ext in ${exts}; do
  if [ ${ext} == "geofence" ]; then
    ...
    baseUrl=${baseAresUrl}
  elif [ ${ext} == "geogig" ]; then
    filename="geoserver-${majorVersion}-SNAPSHOT-geogig-plugin.zip"
    baseUrl=${baseAresUrl}
  elif [ ${ext} == "mbstyle" ]; then
    filename="geoserver-${majorVersion}-SNAPSHOT-mbstyle-plugin.zip"
    baseUrl=${baseAresUrl}
  elif [ ${ext} == "mbtiles" ]; then
    filename="geoserver-${majorVersion}-SNAPSHOT-mbtiles-plugin.zip"
    baseUrl=${baseAresUrl}
  else
